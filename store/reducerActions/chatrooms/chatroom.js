...
const chatroomReducerActions = {
  // Synchronous actions
  sync: {
    GET_MESSAGES__STARTED: (state) => ({...state, fetchingCurrentConvo: true}),
    GET_MESSAGES__FAILED: (state, {payload}) => ({
      ...state,
      fetchingCurrentConvo: false,
      fetchingCurrentConvoError: payload,
    }),
    ...
    ADD_MESSAGE__STARTED: (state) => ({...state, updatingCurrentConvo: true}),
    ADD_MESSAGE__FAILED: (state, {payload}) => ({
      ...state,
      updatingConvo: false,
      updatingConvoError: payload,
    }),
  ...
  async: {
    GET_MESSAGES: ({dispatch}) => async ({payload}) => {
      try {
        ...
        const res = await axios.get(`/api/chatrooms/${chatId}/messages`, {
          params: {
            id: chatId,
          },
        });
    ...
    ADD_MESSAGE: ({dispatch}) => async ({payload}) => {
      try {
        ...
        const res = await axios.post(
          `/api/chatrooms/${message}/messages`,
          payload,
        );
